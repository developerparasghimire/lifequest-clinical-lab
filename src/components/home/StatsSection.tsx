...
export default function StatsSection() {
  return (
    <section className="py-20 relative overflow-hidden" style={{ background: "#020517" }}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <Reveal direction="up">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-white leading-tight h-display">
              Precision Testing{" "}
              <span style={{ color: "#00B67A" }}>You Can Trust</span>
            </h2>
              ...
              {whyUs.map((w) => (
                <div key={w.title} className="flex gap-5 group cursor-default">
                  <div
                    ...
                    style={{ background: "rgba(0,182,122,0.18)", border: "1px solid rgba(0,182,122,0.35)" }}
                  >
                    {w.icon}
                  </div>
                  <div>
              ...
              <RevealItem key={s.label}>
                <div
                  ...
                  onMouseEnter={(e) => {
                    ...
                    el.style.borderColor = "rgba(0,182,122,0.5)";
                    el.style.boxShadow = "0 18px 50px -12px rgba(0,182,122,0.40)";
                    el.style.background = "rgba(0,182,122,0.08)";
                  }}
                  onMouseLeave={(e) => {
                    const el = e.currentTarget as HTMLElement;
                ...
                >
                  <p className="text-5xl font-black text-white mb-2 transition-colors duration-300 group-hover:text-[#00B67A]">
                    <Counter value={s.value} suffix={s.suffix} compact={!!s.compact} />
                  </p>
                  <p className="text-sm" style={{ color: "rgba(255,255,255,0.55)" }}>{s.label}</p>
                  <div className="mt-4 h-0.5 w-10 rounded transition-all duration-300 group-hover:w-20" style={{ background: "#00B67A" }} />
                </div>
              </RevealItem>
            ))}
          </Reveal>
